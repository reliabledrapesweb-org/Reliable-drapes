...
#### 3. E-Catalogue Components (`src/components/eCatalogueComponents/`)

- **`ProductCard.tsx`**: Displays product information (image, title, price) with hover effects.
- **`ProductGrid.tsx`**: Responsive grid layout for displaying a collection of `ProductCard` components.
- **`FilterSidebar.tsx`**: Sidebar component for filtering products by category, price, etc.
...
### E-Catalogue Implementation

- **Objective**: Create a browsable product catalogue with filtering and search capabilities.
- **Changes**:
  - Created `e-catalogue` page route.
  - Implemented `ProductCard`, `ProductGrid`, and `FilterSidebar` components.
  - Added `Breadcrumb` and `SearchBar` for improved navigation and usability.
