...
export function Header() {
    ...
    const ActionButtons = ({ isMobile = false }: { isMobile?: boolean }) => (
        <>
            ...
            >
                <Search className={isMobile ? "h-4 w-4 md:h-5 md:w-5" : "h-4 w-4 xl:h-5 xl:w-5"} />
            </motion.button>
            ...
            <motion.button
                ...
                transition={{ duration: 0.2 }}
            >
                <ShoppingCart className={isMobile ? "h-4 w-4 md:h-5 md:w-5" : "h-4 w-4 xl:h-5 xl:w-5"} />
                {totalItems > 0 && (
                    <span
                        className={isMobile 
                            ? "absolute -top-1.5 -right-1.5 bg-[#2f2581] text-white flex h-3.5 w-3.5 items-center justify-center rounded-full text-[10px] md:h-4 md:w-4 md:text-xs"
                            : "absolute -top-2 -right-2 bg-[#2f2581] text-white flex h-4 w-4 items-center justify-center rounded-full text-xs"
                        }
                    >
                        {totalItems}
                    </span>
        ...
        user ? (
            <div className="relative">
                <motion.button
                    ...
                    transition={{ duration: 0.2 }}
                >
                    <User className={isMobile ? "h-5 w-5 md:h-6 md:w-6" : "h-5 w-5 xl:h-6 xl:w-6"} />
                </motion.button>
                ...
                {showUserMenu && (
                    <motion.div
                        ...
                        transition={{ duration: 0.2 }}
                        className="absolute right-0 mt-3 w-56 rounded-lg border border-gray-200 bg-white shadow-xl overflow-hidden z-[110]"
                    >
                        {/* User Info Header */}
            ...
            <motion.a
                ...
                className={`${
                    shouldUseWhiteText ? "text-white" : "text-black"
                } cursor-pointer text-sm tracking-tight xl:text-base hidden lg:inline-block`}
    ...
    return (
        <header className="fixed top-0 left-0 z-[100] w-full">
            ...
            <MobileMenu isOpen={isOpen} setIsOpen={setIsOpen} navLinks={NAV_LINKS} user={user} />
